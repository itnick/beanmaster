	...
	$.ajax({
		...
		success: function (data) {
			if (!data.err) {
                ...
                tds.eq(4).html(data.stat['current-jobs-urgent'] + data.stat['current-jobs-ready'] + data.stat['current-jobs-reserved'] + data.stat['current-jobs-delayed'] + data.stat['current-jobs-buried']);
